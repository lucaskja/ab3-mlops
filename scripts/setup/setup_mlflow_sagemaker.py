...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Setup MLFlow tracking server on SageMaker')
    
    # Server management
    parser.add_argument('--create-server', action='store_true',
                       help='Create MLFlow tracking server on SageMaker')
    parser.add_argument('--update-tracking-uri', action='store_true',
                       help='Update MLFlow tracking URI in project config')
    parser.add_argument('--delete-server', action='store_true',
                       help='Delete MLFlow tracking server')
    
    # Server configuration
    parser.add_argument('--instance-type', type=str, default='ml.m5.large',
                       help='SageMaker instance type for MLFlow server')
    parser.add_argument('--artifact-bucket', type=str,
                       help='S3 bucket for MLFlow artifacts')
    parser.add_argument('--artifact-prefix', type=str, default='mlflow-artifacts',
...
def create_mlflow_server(args, project_config):
    ...
    try:
        ...
        if not artifact_bucket:
            ...
            if not artifact_bucket:
                artifact_bucket = f"{project_config['project']['name']}-mlflow-artifacts"
...
def delete_mlflow_server(args, project_config):
    ...
    try:
        ...
        update_project_config("http://localhost:5000", 
...
def update_tracking_uri_only(args, project_config):
    ...
    try:
        ...
        # Update project config
        update_project_config(tracking_uri, 
                             project_config['mlflow']['artifact_bucket'],
                             args.artifact_prefix)
...
def main():
    ...
    try:
        ...
        if args.create_server:
            ...
            logger.error("No action specified. Use --create-server, --update-tracking-uri, or --delete-server")
            return 1
